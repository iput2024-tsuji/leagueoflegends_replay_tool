...
1. 目的、完了条件、影響範囲、テスト観点をIssueへ記載します。
2. Issue単位で1目的のブランチを作成して実装します。
3. 必要な自動テストを実行し、PRを作成します。
4. レビュー指摘を重要度別に整理し、修正または見送り理由を記録します。
5. 実環境に依存する変更は、Windows実機で必要な手動確認を行います。
6. CI、レビュー、必要な実機確認が完了してからSquash mergeします。
7. 配布が必要な変更をまとめた場合だけRelease手順へ進みます。

手動確認の選び方は [`docs/manual-test-checklist.md`](docs/manual-test-checklist.md)、
主要な実装箇所と処理フローは [`docs/development-map.md`](docs/development-map.md) を参照してください。
...
- マージ後は作業ブランチを削除します。
- PRは関連Issueを明記し、レビュー指摘を `Must`、`Should`、`Nit` に分類します。
- 対応しない指摘がある場合は、理由と必要に応じてフォローアップIssueを記載します。
