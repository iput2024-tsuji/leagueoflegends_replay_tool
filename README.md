...
- TFTと判定できた試合は録画せず、次のLoLの試合を監視する
...
- `riot_api.py` / `RiotAPIClient`: Live Client API と LCU からのデータ取得、レスポンスのパース、poll 状態の区別を担当
- `recorder_config.py` / `AppConfig`: 録画設定の構造化、読み込み、ユーザーデータ配下のパス解決を担当
- `storage_policy.py`: 容量上限、アプリ所有の録画判定、上限超過時の安全な削除を担当
...
src/
  ...
  recordtest.py           # 録画状態遷移、セッション統合、既存 import の互換 facade
  recorder_config.py      # 録画設定の構造化、読み込み、パス解決
  riot_api.py             # Live Client API / LCU クライアントと poll 状態の区別
  storage_policy.py       # 容量上限とアプリ所有録画の安全な削除
...
- ゲーム音声はOBS標準のApplication Audio CaptureでLoL用に独立取得し、録画トラック1へ収録します。映像用ウィンドウキャプチャ（WGC）の音声とデスクトップ全体の音声取得は無効です
- マイクは設定画面で選んだデバイスからゲーム音声と同じ録画トラック1へ収録します。「無効（マイクを録音しない）」は録画シーンのマイクだけを収録対象から外し、ミュート設定は保持します。Windows側のデバイスは無効化しません
- 録画中に設定画面を開いても録画は継続し、マイクのデバイス・音量・ミュートだけを変更できます（変更は自動反映され、「保存」もマイク設定だけを反映します）。一般設定を変更する場合は、録画終了後に設定画面を開き直してください
- Application Audio Captureに非対応のOBS、またはゲーム音声ソースの設定に失敗した場合は、エラーを表示して録画を開始しません
