...
process leaseの読み取り、探索、作成、通常終了、stale owned cleanup、明示clearは、thread内lock、専用の永続process間lock `.lol_replay_obs_lease.lock`、固定したroot／file handleの順で同じtransactionに入ります。書き込みは同じroot handleに対する排他的な `.lol_replay_obs_lease.tmp.<32 lowercase hex>` を`fsync`してからno-clobberでatomic publishし、確定した同じdescriptorからbytesを再読込します。既存leaseは上書きせず、新規起動が競合に負けた場合はその新しい`Popen`だけをcleanupします。強制終了で残った厳密形式の一時fileは次のtransactionで同じhandleから回収し、予約prefixの不正な名前、物理identity／root binding不一致、回収不能はfail-closeします。管理rootとcontrol namespaceがともに存在しない読み取りはrootやlockを作成しません。回復案内にはleaseとlockの絶対pathを併記し、すべてのOBSと関連toolを終了してから再試行するよう示します。
