...
2. 管理対象外OBSがないことを確認して管理対象OBSを停止し、実際に全processが終了したことを再確認する。停止前後で全snapshot、profile directory、欠落していた`basic.ini`を再検証し、OBS終了時flushや競合変更を検知した場合はそのbytesを上書きしない。
