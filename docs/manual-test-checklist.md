...
- [ ] TFTの開始・途中起動・再接続で録画を開始せず、録画開始通知・動画・不要なsession JSONを作らない（試合開始前に管理者へ交代）
- [ ] TFT終了後も監視が続き、次の通常LoLで録画開始・終了・再生・同期が動作する
- [ ] TFT判定に使ったqueue・game mode・phaseの取得経路と結果を記録する。分類情報が欠損した場合は既存fallbackの範囲を記録し、除外成功と扱わない

## ゲーム音声とマイク

音声経路を変更した場合は、実OBSでの設定確認と録画内容の聴取を行います。音声ストリームの存在やデコード成功だけで完了とせず、未確認の項目はIssue／PRに残し、必要な実機確認が完了するまでmerge・Releaseへ進みません。Release公開には別途管理者の明示指示が必要です。

- [ ] Windows build、OBS version、対象コミットを記録し、実OBSの`GetInputKindList`に`wasapi_process_output_capture`（Application Audio Capture）があることと、LoLのゲームprocessを対象に構成できることを確認する
- [ ] 管理対象シーンにゲーム音声ソースが1件だけあり、映像用WGCの`capture_audio`とグローバルのデスクトップ音声が無効で、ゲーム音声・有効なマイクの出力先が録画トラック1であることを実OBS設定で確認する
- [ ] 録画を実際に再生し、ゲーム音声とマイクの発話をそれぞれ聴き分けられること、映像が欠落しないこと、試合序盤・中盤・終盤で音声と映像およびイベントジャンプの同期が保たれることを確認し、確認位置を記録する
- [ ] アプリの「無効（マイクを録音しない）」で対象マイクのscene itemだけが無効になり、WASAPIのdevice_idへdisabledを送らず、ゲーム音声が収録されることを確認する。ミュート値の保持、別デバイス選択による再有効化、保存・再起動後の無効状態も確認する（Windows側のデバイス無効化とは区別する）
- [ ] 録画中に設定を開き、マイクのミュート切り替え→「保存」→メイン画面へ戻る操作を行い、同じ録画が継続し、管理対象OBSのPIDが変わらず、シーン・録画出力の再構成や録画停止が起きないことを確認する
- [ ] 録画中はマイクのデバイス・音量・ミュートだけを変更でき、自動反映と「保存」がマイク設定だけを反映すること、一般設定は録画終了後に設定画面を開き直して変更できることを確認する
- [ ] 音声デバイス一覧の取得が遅い場合やマイクのデバイス・音量・ミュートを連続変更した場合も、最後に選んだ値が画面・保存設定・OBSに反映され、遅れて届いた結果で以前の値へ戻らないことを確認する。確認後はマイク設定を元へ戻して保存する
- [ ] ロード画面から試合中へのウィンドウ遷移と、試合途中でアプリを起動する場合の両方で、録画開始が成功し、映像・ゲーム音声・イベント同期を確認できる
- [ ] ゲームの再接続とOBS WebSocket再接続後に、同じ試合の映像・ゲーム音声が取得でき、音声ソースや録画が重複しないことを確認する
- [ ] 試合終了後に次の試合を録画し、前の試合のprocessへ取り残されず、新しい試合のゲーム音声とマイクを収録できることを確認する
- [ ] マイク無効またはヘッドセット使用で、別アプリのテスト音が録画へ混入せず、ゲーム音声に二重音・エコーがないことを確認する
- [ ] 非対応OBSまたは破棄可能な専用環境でゲーム音声ソースの作成／更新・録画トラック設定を失敗させ、録画を開始せず、録画中の表示や開始通知を出さず、原因を確認できるエラーが残ることと、既存の録画・JSONを変更しないことを確認する
...
- [ ] 破棄可能な専用OBSで録画中の制御接続だけを切断し、同じ所有process・接続先を確認できる場合に録画状態を再取得し、二重開始や出力再構成なしで停止・動画との結び付けができる
- [ ] 専用OBSでStopRecordの応答喪失を注入し、停止を再送せず、動画pathを推測せず、部分失敗の記録・失敗通知となることと、既存動画・JSONを保持することを確認する
- [ ] 正常終了・キャンセル・エラー・CLIのCtrl+Cで、録画停止やJSON保存の失敗を正常完了へ繰り上げず、停止不明のまま次の録画を開始しない（通常キャンセルと停止済みの部分保存は従来どおり扱う）
...
- [ ] 同一試合への再接続でゲームが一時停止した場合、解除前後のイベントが各対応場面の5秒前へ移動し、停止前のイベントもずれない。操作時刻・ゲーム時計・動画位置と保存JSONの`sync_intervals`を照合する
- [ ] 再接続中などの時刻対応が未確認のイベントは誤った位置へジャンプせず、手動同期の案内を出す。「現在位置で同期」がそのイベントだけへ適用され、他の未確認イベントを同期済みとしない
- [ ] 時刻対応のある新しい録画の手動補正、一時停止なしの通常録画、途中起動、`sync_intervals`を持たない既存録画の従来同期を確認し、再生する録画を替えた際に前の手動補正を持ち越さない
